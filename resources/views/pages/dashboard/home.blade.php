    ...
    @role(config('rbac.role.default'))
        <div class="max-w-7xl mx-auto" x-data="iotDashboard()">
            ...
            @foreach ($blocks as $block)
                <x-user.block-accordion-card :blockId="$block['blockId']" :blockName="$block['blockName']" :avgHumidity="$block['avgHumidity']" :avgFlowRate="$block['avgFlowRate']"
                    :totalVolume="$block['totalVolume']" :sprayers="$block['sprayers']" />
            @endforeach
