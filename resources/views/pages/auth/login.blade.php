...
<head>
    ...
    <title>Login Akun - {{ config('app.name', 'Laravel') }}</title>
...
<body class="min-h-screen w-full flex items-center justify-center bg-cover bg-center bg-no-repeat"
    ...
    <div class="w-[640px] h-auto p-12 flex flex-col gap-6 rounded-3xl border-2 border-white bg-[rgba(255,255,255,0.72)] backdrop-blur-md box-border"
        ...
        <!-- Blocked Warning (Shows when rate limited) -->
        <template x-if="isBlocked && remainingSeconds > 0">
            <div class="w-full bg-red-50 border-2 border-red-400 text-red-800 px-5 py-4 rounded-lg">
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex-1">
                        <p class="font-semibold text-sm mb-1">Akun Terkunci</p>
                        <p class="text-sm">Terlalu banyak percobaan login gagal. Coba lagi dalam: 
                            <span x-text="remainingSeconds" class="font-bold text-red-900"></span> detik
                        </p>
                    </div>
                </div>
            </div>
        ...
        <!-- Attempts Warning (Shows remaining attempts when > 3 attempts) -->
        @if (session('show_warning') && session('attempts_left') && session('attempts_left') > 0)
            <div class="w-full bg-yellow-50 border-2 border-yellow-400 text-yellow-800 px-5 py-4 rounded-lg">
                <div class="flex items-center gap-3">
                    ...
                    <p class="text-sm">
                        <strong>Peringatan:</strong> Sisa percobaan login: 
                        <span class="font-bold">{{ session('attempts_left') }}</span> kali lagi
                    </p>
                </div>
            </div>
        @endif
        ...
        <!-- Login Form -->
        <form method="POST" action="{{ route('login.store') }}" class="w-full flex flex-col gap-6"
            ...
            <button type="submit" :disabled="isBlocked && remainingSeconds > 0"
                ...
                <span x-show="isBlocked && remainingSeconds > 0" x-cloak
                    ...
                    Terkunci (<span x-text="remainingSeconds" class="font-mono font-bold my-auto"></span>s)
                </span>
            </button>
        </form>
    </div>
    ...
    <script>
        // Data dari PHP
        const BLOCKED_UNTIL = {{ $blockedUntil ?? 0 }};
                ...
                startCountdown() {
                    ...
                    this.intervalId = setInterval(() => {
                        ...
                        if (this.remainingSeconds <= 0) {
                            ...
                            // Auto reload untuk clear session
